...
Double_t* countEvents(Double_t fluxes[], Double_t areas[]){
   ...
   auto scalingfactor = pow(470./Icarus_dist,2) * 1e-42 * totAr * (6e20*1e-6); //(470/600)^2 = scaling factor for distance (data normalized at 470 m from source); 1e-4 converting cm^2 to m^2; 6*10^20 protons on-target     
